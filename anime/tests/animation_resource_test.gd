...
func _run() -> void:
	...
	if failures.is_empty():
		print("ANIMATION_RESOURCE_TEST_PASS checks=%d source_frames=783 generated_frames=886 profiles=7" % checks)
		quit(0)
		return
...
func _test_manifest_and_frames() -> void:
	...
	_check(source_total == 783, "source frame total is 783, got %d" % source_total)
	...
	_check(generated_total == 886, "generated frame references total is 886, got %d" % generated_total)
	for empty_actor in ["rocky", "sharkk", "core_body"]:
		var empty_frames := load("res://anime/generated/%s_frames.tres" % empty_actor) as SpriteFrames
		_check(empty_frames != null, "%s placeholder SpriteFrames loads" % empty_actor)
...
func _test_profiles() -> void:
	...
	for visual_profile in ["player", "boom", "core00_true_hand", "core00_false_hand"]:
		_check((profiles_by_id[visual_profile] as BattleAnimationProfile).has_visual_frames(), "%s has visual frames" % visual_profile)
	for fallback_profile in ["rocky", "sharkk", "core00_body"]:
		_check(not (profiles_by_id[fallback_profile] as BattleAnimationProfile).has_visual_frames(), "%s safely uses glyph fallback" % fallback_profile)
	...
	for profile_id: String in profiles_by_id:
		if profile_id != "boom":
			_check((profiles_by_id[profile_id] as BattleAnimationProfile).audio_cues.is_empty(),

...
/// <summary>
/// NPC 基类。在可互动基础上提供 Dialogic 对话接入。
/// 只需在 Inspector 拖入 Timeline 与 Character；气泡 Canvas 和角色锚点会自动准备。
/// 对话内容、条件、选项、变量与流程全部由 Dialogic 负责。
/// </summary>
[GlobalClass]
public abstract partial class NPCBase : InteractableBase
{
    ...
    [Signal] public delegate void DialoguePluginSignalEventHandler(Variant argument);

    private const string DialogueCanvasScenePath =
        "res://features/dialogue/scenes/npc_dialogue_canvas.tscn";
    private const string DialogueBubbleScenePath =
        "res://features/dialogue/scenes/dialogue_bubble.tscn";
    private const string GeneratedAnchorName = "DialogicBubbleAnchor";
    private const string GeneratedPlayerAnchorName = "DialogicPlayerBubbleAnchor";
    private const string NpcBubbleColumnName = "NPCBubbleColumn";
    private const string PlayerBubbleColumnName = "PlayerBubbleColumn";
    private const string DialogueInputBlockerName = "DialogueInputBlocker";
    private const int DialogueModalCanvasLayer = 100;

    // Dialogic 是全局单例。这个 owner 锁避免两个 NPC 同帧都 CallDeferred 启动 Timeline。
    private static NPCBase _dialogueOwner;

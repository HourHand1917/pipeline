...
const CATALOG := preload("res://Resource/card/card_catalog.gd")
...
func _test_upgrade_catalog() -> void:
	var bases: Array[CardData] = CATALOG.load_all()
	var upgrades: Array[CardData] = CATALOG.load_upgrades()
	var versions: Array[CardData] = CATALOG.load_all_versions()
	_check(bases.size() == 14, "基础卡字典必须包含14张卡")
	_check(upgrades.size() == 14, "升级卡字典必须包含14张卡")
	_check(versions.size() == 28, "完整字典必须包含28个版本")

	var ids: Dictionary = {}
	for card: CardData in versions:
		_check(card != null, "字典内不能出现空卡牌")
		if card == null:
			continue
		_check(not ids.has(card.id), "基础卡和升级卡ID必须唯一：%s" % card.id)
		ids[card.id] = true
		_check(card.resource_path.begins_with("res://Resource/card/"), "卡牌必须位于Resource/card：%s" % card.resource_path)
		_check(not card.effects.is_empty(), "卡牌必须配置可执行效果：%s" % card.id)
		for effect: CombatEffectData in card.effects:
			_check(effect != null, "卡牌效果不能为空：%s" % card.id)
			if effect != null:
				_check(effect.resource_path.begins_with("res://Resource/card/effects/") \
						or effect.resource_path.begins_with("res://Resource/card/upgrade_effects/"), \
					"效果必须位于Resource/card：%s" % effect.resource_path)

	for base: CardData in bases:
		var upgraded := CATALOG.load_upgrade_for(base.id)
		_check(upgraded != null, "基础卡必须配置升级版：%s" % base.id)
		if upgraded == null:
			continue
		_check(base.upgraded_version == upgraded, "基础卡升级引用必须对应字典：%s" % base.id)
		_check(upgraded.id == StringName("%s_up" % base.id), "升级卡ID必须与基础卡对应：%s" % base.id)
		_check(upgraded.is_upgraded, "升级卡必须标记is_upgraded：%s" % upgraded.id)

	var kiss := CATALOG.load_by_id(&"deadly_kiss_up")
	_check(kiss != null and kiss.effects.size() == 2, "致命亲亲+必须保留两段攻击")
	if kiss != null and kiss.effects.size() == 2:
		_check(kiss.effects[0].amount == 6 and kiss.effects[1].amount == 6, "致命亲亲+必须造成6+6伤害")
		_check(kiss.effects[0] != kiss.effects[1], "致命亲亲+两段效果必须是独立资源")
	var pump := CATALOG.load_by_id(&"hemostatic_pump_up")
	_check(pump != null and pump.effects.size() == 1 and pump.effects[0].amount == 8, "止血泵+必须治疗8点")
